                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{ 'Cable plan' }}</h4>
                        </div>
                        <div class="card-content">
                            <div class="card-body">
                                <form action="{{ route('edit_cable') }}" method="POST">
                                    @csrf
                                    <div id="error_result">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 col-12">
                                            <div class="form-group">
                                                <label for="first-name-column">Cable Name</label>
                                                <input type="hidden" name="plan_id" value="{{ $Plan->id }}">
                                                <select class="form-control" name="cable">
                                                    @foreach ($Cables as $Cable)
                                                        <option @if ($Cable->id == $Plan->cable_id) selected @endif
                                                            value="{{ $Cable->id }}">{{ $Cable->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-12 col-12 mt-4 mb-4">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="selling_price" name="selling_price"
                                                    class="form-control" value="{{ $Plan->plan_amount }}">
                                            </div>
                                        </div>

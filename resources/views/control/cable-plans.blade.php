        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Cable Plans </h3>
                    <p class="text-subtitle text-muted">View and manage cable plans on the system</p>
                </div>
        ...
        <section class="section">
            <div class="card">
                <div class="card-header">
                    Cable plan records
                    <a href="{{ route('create-cable') }}" class="btn btn-primary">Create New Plan</a>
                </div>
                <div class="card-body">
                    <table class="table table-striped" id="table1">
                        ...
                        <tbody>
                            ...
                            @foreach ($Pricing as $index => $Price)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td><strong>{{ strtoupper($Price->cable_name) }}</strong></td>
                                    <td>{{ $Price->plan_name }}
                                    <td>{{ $Price->plan_code }}</td>
                                    <td>{{ number_format($Price->cost_price, 2) }}</td>
                                    <td>{{ number_format($Price->plan_amount, 2) }}</td>
                                    <td>
                                        <span
                                        ...
                                        <div class="btn-group mb-1">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButtonIcon">
                                                    <a class="dropdown-item" href="/pricing/cable/{{ $Price->id }}"
                                                        href=""> View</a>
                                                    <a class="dropdown-item" href="/cable/delete/{{ $Price->id }}" confirm="Are you sure to delete this record ?">
                                                        Delete</a>
                                                </div>
                                            </div>
                                        </div>
